<?php 
    ...
    if(isset($_POST["upload"])) {
        $img = $_FILES["choose"];
        $result_pic = upload_profile_photo($img, $_SESSION["uid"]);
    }
    ...
    if(isset($_POST["changePass"])) {
        ...
        $pass_veri = htmlspecialchars($_POST["veri_pass"]);
        $result_pass = change_pass($_SESSION["uid"], $pass, $pass_veri);
    }
...
<div class="wrapper">
    ...
    <div class="container">
	<h2 style="margin: 15px;">Settings</h2>
        <div id="profilePic" style="padding: 10px;">
            ...
            <div class="row">
                ...
                <div class="gallery d-flex flex-row align-items-center">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <input type="submit" name="upload" id="upload" class="btn btn-primary px-3" value="Upload" />
                    </form>
                </div>
            </div>
            <div class="row">
                <?php 
                if($result_pic === true) echo "<h5>Success!</h5>\n";
                else if($result_pic === false) echo "<h5>Error uploading photo</h5>\n";
                ?>
            </div>
        </div>
        <div id="passwordChange" style="padding: 10px;">
            <h4>Change password</h4>
            <form method="POST">
                <input type="password" name="pass" class="form-control" placeholder="password" required /><br>
                <input type="password" name="veri_pass" class="form-control" placeholder="verify" required /><br>
                <input name="changePass" type="submit" class="btn btn-primary" value="Change password" />
            </form>
            <?php 
                if($result_pass === true) echo "<h5>Success!</h5>\n";
                else if($result_pass === false) echo "<h5>Error changing password</h5>\n";
                ?>
        </div>
        <div id="deleteAccount" style="padding: 10px;">
            <h4>Delete user account</h4>
            <form method="POST">
                <input type="submit" name="deleteAcc" class="btn btn-primary" value="Delete account" />
            </form>
        </div>
    </div>
...
$("#choose").change(function() {
  ...
  // and just enough to fit minimum of 28 photos in kitlab db
